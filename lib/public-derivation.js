...
const DERIVATION_VERSION = 'pub-6';
...
async function discardStaleDerivations(pool, inputKey) {
  ...
  try {
    ...
    await client.query('DELETE FROM public_receipts');
...
// A system-metadata receipt, projected from its staged row. The private path
// is consulted for exactly one thing — which operating system wrote the file —
// and the answer is a fixed vocabulary entry, never the string itself. A
// receipt this build does not recognise stops the derivation rather than
// being published under an invented label.
function projectReceipt(record) {
  const basename = String(record.source_path || '').split('/').pop();
  const receiptKind = basename === '.DS_Store' ? 'macos_folder_metadata' : null;
  if (!receiptKind) {
    throw new Error(
      `public projection: the receipt at sequence ${record.source_sequence} is a system file this build does not recognise`);
  }
  const row = {
    ref: 'r' + record.source_sequence,
  ...
  P.assertPublicSafe(row, `receipt ${row.ref}`);
  return row;
}
...
async function finalize(pool, { inputKey }) {
  ...
  try {
    ...
    // The 27 system-metadata receipts, so the catalogue can account for every
    // audited item, not just the substantive sources. They carry canonical
    // facets only — no filename, no path — and the whole set lands in the
    // same transaction as the stage's completion marker.
    const receiptRows = await client.query(
      `SELECT source_sequence, source_path, disposition, corpus_role, recommended_use, bytes
         FROM v13_sources WHERE material_type = 'system_metadata' ORDER BY source_sequence`);
    for (const receipt of receiptRows.rows) {
      const row = projectReceipt(receipt);
      await client.query(
        `INSERT INTO public_receipts
           (ref, source_sequence, receipt_kind, disposition, corpus_role, recommended_use, bytes)
         VALUES ($1, $2, $3, $4, $5, $6, $7)
         ON CONFLICT (ref) DO NOTHING`,
        [row.ref, receipt.source_sequence, row.receipt_kind, row.disposition,
         row.corpus_role, row.recommended_use, row.bytes]);
    }
