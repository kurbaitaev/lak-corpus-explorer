...
function createCorpusV2Bootstrap({ env = process.env, migrateFiles, importBundle, importLexiconBundle, reconcile, pool } = {}) {
  return async function bootstrap({ before } = {}) {
    ...
    await migrateFiles();
    const lexicon = await importLexiconBundle(undefined, { migrate: false });
    if (!strictTrue(env.CORPUS_V2_AUTO_IMPORT)) {
      return { ran: true, migrated: true, lexicon, reason: 'lexicon imported; PCMLBE auto-import is not enabled' };
    }
    ...
    return { ran: true, batchId: imported.batchId, idempotent: imported.idempotent === true, lexicon };
  };
}
...
function defaultDeps() {
  ...
  const { importLexiconBundle } = require('../scripts/import-lexicon-synthesis');
  const { EXPECTED } = require('../scripts/reconcile-corpus-v2');
  const { pool } = require('./db');
  return {
    migrateFiles,
    importBundle, importLexiconBundle,
