...
// ── v1.3 ───────────────────────────────────────────────────────
async function prepareV13(pool, pkg, availability) {
  const dir = availability.cache_dir;
  if (!availability.present) {
    return {
  ...
  if (cached) {
    ...
    const digests = (cached.observed || {}).file_digests || {};
    const batches = await pool.query('SELECT layer, file_sha256 FROM v13_import_batches');
  ...
  const staged = await v13.importPackage(pool, dir, verification);
  await packages.writeCachedVerification(pool, pkg.id, fingerprint.key, {
...
// ── Orchestration ──────────────────────────────────────────────
async function preparePrivatePackages(pool) {
  ...
  for (const pkg of packages.PACKAGES) {
    ...
    try {
      prepared = pkg.id === 'v1.2'
        ...
        : await prepareV13(pool, pkg, availability);
    } catch (err) {
      prepared = {
