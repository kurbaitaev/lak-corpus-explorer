...
function extractArchive(archivePath, destDir, prefix) {
  ...
  const stageDir = prefix
    ...
    : destDir;
  const result = spawnSync('unzip', ['-q', '-o', archivePath, '-d', stageDir], { stdio: 'pipe' });
  if (result.status !== 0) {
    if (prefix) fs.rmSync(stageDir, { recursive: true, force: true });
    throw new Error(`archive could not be extracted: ${String(result.stderr || '').trim() || 'unzip failed'}`);
  }
  if (prefix) {
...
async function ensureLocalCache(pool, pkg) {
  ...
  try {
    ...
    extractArchive(archivePath, dir, pkg.archive_prefix);
    if (!cacheComplete(pkg, dir)) {
      return {
