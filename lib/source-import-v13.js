...
// Batching, commit cadence and the resumable writer itself live in
// lib/chunk-writer.js: the public projection derivation needs exactly the same
// behaviour, and one implementation is easier to reason about than two.
...
async function importLayer(pool, { packageDir, layerId, file, declaredCount, sql, columns, run }) {
  ...
  const writer = new ChunkWriter(pool, {
    sql, columns, resumeFrom,
    recordProgress: (client, index, final) => client.query(
      `UPDATE v13_import_batches
          SET resume_offset = $2, imported_count = $2, status = $3, updated_at = now()
        WHERE id = $1`,
      [batchId, index, final ? 'complete' : 'in_progress']),
  });
  // The row builders stamp each staged row with the batch that produced it.
