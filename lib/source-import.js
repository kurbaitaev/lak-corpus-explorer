...
function publicStatus(verification) {
  return {
    ...
    sources: verification.sources.map(s => ({
      ...
      // `records` is empty when the verification was reused from cache, so the
      // count travels alongside it rather than being derived from the array.
      verified_record_count: s.status === 'verified'
        ? (s.verified_record_count ?? s.records.length)
        : 0,
      metrics: s.metrics,
