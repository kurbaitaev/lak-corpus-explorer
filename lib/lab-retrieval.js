...
const { createAliasResolver, containsRussianForm } = require('./russian-morphology');
...
function createRetriever(deps) {
  ...
  const resolveAlias = createAliasResolver([curatedAliases, corpusAliases], norm);

  // Original (un-normalized) alias strings for a normalized RU query.
  function aliasesFor(normQuery) {
    return resolveAlias(normQuery).aliases;
  }
  ...
  function retrieveRu2Lak(query, { limit = 25 } = {}) {
    ...
    for (const r of corpusData) {
      if (r[0] === 'lexicon') {
        ...
        const exactGloss = gloss === q || tokenHas(r[2], q) || containsRussianForm(r[2], q);
